...
-- 마감 임박 알림 중복 방지: 알림을 보낸 목표일('YYYY-MM-DD'). 목표일이 바뀌면 다시 알림.
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS due_notified_for text;

-- 파일 링크(클라우드 저장소 주소) — [{url, label}] 배열. 업무/입사자/퇴사자에 부착.
ALTER TABLE tasks       ADD COLUMN IF NOT EXISTS links jsonb NOT NULL DEFAULT '[]';
ALTER TABLE onboarding  ADD COLUMN IF NOT EXISTS links jsonb NOT NULL DEFAULT '[]';
ALTER TABLE offboarding ADD COLUMN IF NOT EXISTS links jsonb NOT NULL DEFAULT '[]';

-- 퇴사자: 재입사 예정 여부 / 입퇴사 개인별 메모
ALTER TABLE offboarding ADD COLUMN IF NOT EXISTS rehire_planned int NOT NULL DEFAULT 0;
ALTER TABLE onboarding  ADD COLUMN IF NOT EXISTS memo text;
ALTER TABLE offboarding ADD COLUMN IF NOT EXISTS memo text;

-- ===== 앱 설정(키-값 JSON) — 업무 구분/체크리스트 옵션 등 사용자 편집 가능 설정 =====
...
ALTER TABLE task_todos ADD COLUMN IF NOT EXISTS due_date text;
...
-- 정기 규칙 확장: 생성 시 함께 만들 세부 To-Do 프리셋(문자열 배열) / 업무 세트 연결
ALTER TABLE recurring_rules ADD COLUMN IF NOT EXISTS todos jsonb NOT NULL DEFAULT '[]';
ALTER TABLE recurring_rules ADD COLUMN IF NOT EXISTS preset_id bigint;

-- ===== 업무 세트(패키지 프리셋) =====
-- content: { project: {title, category, priority}, tasks: [{title, subcategory, priority, offset, todos: []}] }
...
-- ===== 텔레그램 알림 =====
-- tg_chat_id: 봇과 1:1 대화방 id. 연결되면 인앱 알림과 같은 내용을 텔레그램으로도 보낸다.
-- tg_code: 연결용 1회성 코드(만료 시각 tg_code_at). /start <code> 로 본인 확인.
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_chat_id text;
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_on      int NOT NULL DEFAULT 1;
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_code    text;
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_code_at timestamptz;
CREATE INDEX IF NOT EXISTS idx_users_tg_code ON users(tg_code);
`;

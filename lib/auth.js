...
export async function getSessionUser(token) {
  ...
  const row = await one(
    `SELECT s.expires_at, u.id, u.username, u.name, u.role, u.must_change_pw
  ...
  return { id: row.id, username: row.username, name: row.name, role: row.role, must_change_pw: !!row.must_change_pw };
}
